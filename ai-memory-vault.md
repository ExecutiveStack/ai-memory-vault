...
4. **Find or create the vault, and never create over what exists.** Obsidian keeps a registry of every vault on the machine in its app config, `obsidian.json` (macOS: `~/Library/Application Support/obsidian/obsidian.json` · Windows: `%APPDATA%\obsidian\obsidian.json` (lowercase folder) · Linux: `~/.config/obsidian/obsidian.json`). Read it: it lists vault names and paths only, never note contents, so it sits inside the privacy rules. Then:
   ...
   - **The home-folder rule is for a vault YOU create in this run. An existing vault is right where it is, wherever that is.** Never comment on where an existing vault lives, never suggest it should be somewhere else, never mention this rule when listing or pointing at one, even if it sits in Documents, Desktop, or a cloud folder. They chose it, it works, and the day they need it moved there is a tool for that and they will ask. Location comes up exactly twice: when you create a fresh vault, and when the person asks.
5. **Register the vault so Obsidian opens straight into it. Never skip this.** Obsidian only knows about vaults listed in `obsidian.json` (the same registry file from step 4). A vault you created on the filesystem is invisible to the app until registered, and an unregistered vault means the person's first launch is a welcome screen asking them to pick a vault they don't know they have. (If they chose an existing vault from the registry, it is already listed; skip the write and just launch.) Quit Obsidian first if it is running, so the app is not writing the file while you are.

   **THIS IS THE MOST DANGEROUS STEP IN THIS GUIDE, and it does not look dangerous.** You are hand-editing a config file belonging to software you did not write, which holds the registry of every vault this person owns. Get it wrong on a machine with real vaults and you can wipe years of someone's registrations. Read all five points before touching the file.

   - **BACK IT UP FIRST, WHATEVER STATE IT IS IN.** If `obsidian.json` exists, copy it to `obsidian.json.bak` before you write — healthy or malformed, no exceptions. Backing up only a *malformed* file protects nobody, because the dangerous case is a perfectly good file you are about to merge into: it passes every check, gets no backup, and there is nothing to restore from. If the file or its folder does not exist yet (the app has never launched), create both; there is nothing to back up.
   - **DO NOT BUILD THIS JSON THROUGH YOUR SHELL.** On Windows the vault path is full of backslashes, a backslash is an escape character inside JSON, and every layer between you and the file — the shell, its quoting, the language you write in — eats one more level of them. Measured on a real install: three consecutive attempts, through `printf`, an unquoted heredoc, and `node -e` inside double quotes, produced three DIFFERENT wrong answers. **Write a script FILE using a quoted heredoc (`<<'EOF'`), which expands nothing, and let a real JSON serializer do the escaping.** Never assemble this file by string substitution.
   - **Merge, never replace:** keep every existing vault entry and add one for this vault, keyed by a fresh 16-character lowercase hex ID, with the value `{"path": "<absolute vault path>", "ts": <current epoch milliseconds>, "open": true}`. The `open: true` is what makes Obsidian land in this vault at launch.
   - **VERIFY BY CHECKING THE PATH EXISTS ON DISK — parsing the file is NOT the test.** Valid JSON never was the test, and here is why, measured: the worst failure on record wrote *perfectly valid JSON* containing `C:Usersjaredhenry-vault`, every separator eaten, and sailed straight through a parse check that reported success. **Take the path string back out of the file you just wrote and confirm something actually exists at it.** A path that does not exist means the write is wrong no matter how good the JSON looks. Then confirm every vault that was listed before is still listed, and that the file gained exactly one entry.
   - **That check must not need an interpreter you do not have.** A clean Windows machine has neither Python nor Node, and `python` there is a Microsoft Store stub that exits without running — so do not reach for one by reflex, or your safety check is the thing that fails. Any shell can test whether a directory exists. If you genuinely cannot verify the write, say so to the person and restore the backup rather than leave an unverified edit in place.
   - Now launch Obsidian (macOS: `open -a Obsidian` · Windows: `start "" "%LOCALAPPDATA%\Programs\Obsidian\Obsidian.exe"` · Linux: however it was installed) and confirm with the person that it opened inside their vault. Tell them what they're looking at: this is their agent's memory, and every note it writes shows up here. On macOS the first launch of a downloaded app may ask them to click Open; that is the system being protective, not a problem.
6. **The backup sentence. Say it every time a fresh vault was created.** A vault in the home folder is not cloud-synced (part of why it lives there: no sync service can quietly swap the agent's memory for internet-only placeholders), so it exists only on this computer, and a lost or dead machine takes the memory with it. Say that once, plainly, and point at the free options in this repo's `TROUBLESHOOTING.md`: Time Machine already covers the home folder on a Mac, and a private GitHub repo the agent can set up works anywhere. If they chose an existing vault, skip this step; its backup arrangements are theirs and you say nothing about them.
7. (Optional — skip it freely.) Obsidian SYNC is a paid add-on that only syncs the vault between devices; it is NOT needed for anything in this system. (Read that carefully: SYNC is the add-on. Obsidian itself is required and already installed by now.) The vault is plain files on your computer and the AI reads them directly. If you ever want multi-device sync later, Sync works (Settings → Core plugins → Sync; standard encryption is fine), and a private GitHub repo does it free.
8. **Turn ON "Automatically update internal links", so renaming a note repairs every link to it.** Do it with no hands and before the first launch: write `{"alwaysUpdateLinks": true}` to `<vault>/.obsidian/app.json`, creating the folder if needed. Obsidian reads it at startup and adds its own files alongside without disturbing yours — verified on a real install. If Obsidian has already launched, use the app instead: Settings → Files & Links.

## Step 2 — Connect Claude to the vault

**Option A — Claude Code (recommended, most capable).** Keep your vault as pure notes and run Claude Code from a *separate* working folder. Make a small folder for your AI (say `~/my-agent`, the same folder name the one-command full installer uses, so every guide in this ecosystem points at the same place), put your `CLAUDE.md` boot config in it, and launch `claude` from there. Claude Code auto-loads that `CLAUDE.md` every session; the boot config carries your vault's path and points the AI at it. From there it reads and writes your vault files directly over the filesystem — no MCP server, no tunnel, nothing to maintain — and the first time it touches the vault folder it will ask for one-time file access; approve it. Keeping `CLAUDE.md` (which is Claude-Code config, not a note) *out* of the vault is what lets the vault stay a clean, tool-agnostic memory you can point any number of projects or AIs at without confusion. This is the two-layer boot (below): CLAUDE.md in your working folder, VAULT-INDEX.md in the vault. The rest of this guide assumes this path.
...
**LISTEN FOR "THIS IS A TEST", AND TAKE THE SHORTCUT WHEN YOU HEAR IT.** Some of the people running this are not building their own memory: they are the author, a developer, or someone kicking the tyres before they commit. A real answer to question one, measured: *"Honestly? I am the author. I am doing a test install on a PC."* Without a branch for that, this phase interviews a person about their own company, their own key people and their own working style for ten minutes, which is absurd to sit through and tells you nothing you can use.

So when someone says they are testing, evaluating, or that they built this: **say you will keep it short, ask only for the agent's name and one line about what they do, then build the COMPLETE structure with honest, light placeholder content.** Every folder, every file, every rule, exactly as a real run would produce — the point of a test is that the machinery gets exercised, so nothing is skipped, only the interrogation. Tell them plainly what you did: "I have built the whole thing with thin content, since you are testing. Fill any of it in whenever you like, or tell me about yourself now and I will do it properly." Then move on.
...
**One memory, not two.** If the person uses Claude Code, it also keeps its *own* per-project memory file that auto-loads every session, separate from the vault (typically `~/.claude/projects/<project>/memory/MEMORY.md`).

**AND `<project>` IS ALMOST CERTAINLY NOT THE ONE YOU ARE RUNNING IN, so read this before you write anything.** Claude Code names these folders after the WORKING DIRECTORY, with every separator turned into a dash, so the folder name is the whole path spelled out in dashes and ends with the directory's own name. On Windows the drive letter and its colon collapse into a double dash at the front, which is the only reason it looks unfamiliar; the rule is identical on every platform. The one you want is **the folder the person's AGENT will run in**, which is the agent's home. If you are running this installer from inside a sub-folder -- a toolkit or repo folder one level down -- then YOUR project folder is a different one, and writing the redirect there changes YOUR OWN behaviour and says nothing at all to their agent. **That failure is silent and looks exactly like success**, because a file called `MEMORY.md` now exists in a folder called `memory`. Measured on a real install, where both folders existed side by side and only one of them was the right one. Confirm the agent's home directory, derive the folder name from THAT, and say out loud which one you wrote to. Left alone, that becomes a second memory layer that drifts from the vault — some things get written here, some there, and the two quietly disagree. This is the single most common way these systems rot. Close it on day one — but if that memory file (or any notes the person already keeps) already holds real content, migrate it into the vault FIRST. Two hard rules govern this entire migration: **you COPY, you never move — and you never delete.** The original memory files stay exactly where they are, untouched. Deleting originals is the person's job alone, by hand, if and when they choose — the most you ever do is tell them the path. The order:
